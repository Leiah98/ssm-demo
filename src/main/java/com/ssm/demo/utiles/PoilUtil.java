...
public class PoilUtil {
     ...
     * @param fields    导出的数据map.get(key)对应的key
     * @param formators 导出数据的样式
     * @param widths    表格的列宽度 默认为512 * 14
     * @param data      数据集 List<Map>
     * @param request
     * @param response
     * @throws IOException
     */
    public static void exportFile(String excelName, String[] headers, String[] fields, int[] formators, int[] widths, List<Map<String, Object>> data, HttpServletRequest request, HttpServletResponse response) throws IOException {
        if (widths == null) {
        ...
        if (formators == null) {
            formators = new int[fields.length];
            for (int i = 0; i < fields.length; i++) {
                formators[i] = 1;
            }
        }
        ...
        if (data != null) {
            ...
            for (int i = 0; i < data.size(); i++) {  //行数
                XSSFRow row = sheet.createRow(headerrow);
                Map map = data.get(i);
                for (int j = 0; j < fields.length; j++) {  //列数
                    XSSFCell cell = row.createCell(j);
                    Object o = map.get(fields[j]);
                    if (o == null || "".equals(o)) {
                        cell.setCellValue("");
                    } else if (formators[j] == 4) {
                        //int
                        cell.setCellValue((Long.valueOf((map.get(fields[j])) + "")).longValue());
                    } else if (formators[j] == 5 || formators[j] == 6) {
                        //float
                        cell.setCellValue((Double.valueOf((map.get(fields[j])) + "")).doubleValue());
                    } else {
