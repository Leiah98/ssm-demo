...
public class AdminUserServiceImpl implements AdminUserService {
    ...
    @Override
    public int importUserByExcelFile(File file) {
        XSSFSheet xssfSheet = null;
        try {
            //读取file对象并转换为XSSFSheet类型对象进行处理
            xssfSheet = PoilUtil.getXSSFSheet(file);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
    ...
    private AdminUser convertXSSFRowToAdminUser(XSSFRow xssfRow) {
        ...
        if (!StringUtils.isEmpty(userName)) {
            adminUser.setUserName(PoilUtil.getValue(userName));
        }
        //对读取的密码进行加密并设置到daminUser对象中
        if (!StringUtils.isEmpty(orinalPassword)) {
            adminUser.setPassword(MD5Util.MD5Encode(PoilUtil.getValue(orinalPassword), "UTF-8"));
        }
        return adminUser;
    }
}
